...
let AC = null;
// 하드뮤트 코디네이터 — 셋 중 하나라도 참이면 AudioContext 정지:
//  ① 탭 백그라운드  ② 광고 재생 중  ③ CrazyGames 플랫폼 muteAudio.
// 인게임 사운드 토글(Bgm.enabled, gain 기반)과 독립된 상위 레이어.
let _adMuted = false, _platformMuted = false;
function applyAudioGate() {
  if (!AC) return;
  const suspend = document.hidden || _adMuted || _platformMuted;
  try {
    if (suspend) { if (AC.state === 'running') AC.suspend(); }
...
function audio() {
  ...
  applyAudioGate(); // 게이트 조건 반영 (예: 플랫폼 뮤트 중이면 resume하지 않음)
  return AC;
}
...
function land() {
  ...
  if (!newBestShown && Storage.data.bestScore > 0 && score > Storage.data.bestScore) {
    ...
    CG.happytime(); // CrazyGames: 신기록 축하 모먼트
  }
...
function gameOver(reason) {
  ...
  mover = null;
  state = 'TOPPLING';
  CG.gameplayStop(); // CrazyGames: 플레이 종료(결과/휴식 구간 진입)
...
function reviveFromWreckage() {
  ...
  spawnMover();
  state = 'AIM';
  CG.gameplayStart(); // CrazyGames: 이어하기로 플레이 재개
}
...
function resetRun(startHeight = DEBUG_START_HEIGHT) {
  ...
  spawnMover();
  state = 'AIM';
  CG.gameplayStart(); // CrazyGames: 새 런 시작 (포털 빌드에서만 동작)
}
