...
- **⚠️ GM에는 리워드 포맷이 없다** — SDK에 rewarded 개념 자체가 없음(`showBanner()` 하나). 서빙되는 광고가 **비디오형이면** IMA 이벤트(`STARTED→…→COMPLETE`)가 오지만, **디스플레이형 전면광고는 COMPLETE 없이** 닫혀 `SDK_GAME_START`만 온다. 그래서 보상 기준은 **"광고가 실제로 표시됨(SDK_GAME_PAUSE = started)"** — 표시 후 종료되면 보상 지급, 무필(started=false)이면 보상 없음. COMPLETE도 여전히 즉시 보상(비디오 완주 시 먼저 도착).
  - 이력: 처음 COMPLETE만 기준으로 했더니 호스팅 환경에서 디스플레이형 광고를 본 유저의 이어하기가 무시됨(2026-07-08 실사용 리포트) → started 기준으로 수정·검증.
