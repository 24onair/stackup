...
- **⚠️ GM에는 리워드 포맷이 없다 → "요청 후 무조건 지급" 정책** — SDK에 rewarded 개념이 없음(`showBanner()` 하나, 완주 검증 불가). 게다가 첫Play 광고 직후 이어하기는 빈도제한으로 무필(이벤트 0개)이 잦다. 그래서 GM 리워드는 **광고를 요청한 뒤 흐름이 끝나면 무조건 보상**: 광고가 뜨면(비디오 COMPLETE / 디스플레이 SDK_GAME_START) 보고 나서 지급, 무필이면 15s 워치독이 지급, 에러도 지급. 수익은 실제 뜬 광고에서 발생하고, 사용자는 이어하기/점수2배가 항상 동작. (CG·GD는 진짜 리워드라 완주 검증 유지 — 이 정책은 GM 전용.)
  - 이력(2026-07-08 실사용): ①COMPLETE만 기준 → 디스플레이형 광고 무시됨 ②started 기준 → 무필 시 여전히 먹통 → **③"요청 후 무조건 지급"으로 확정**. 검증: 디스플레이형·무필·비디오완주 3케이스 모두 이어하기 복귀·이중지급 없음·에러 0.
  - 캐시: 포털 CDN이 JS를 10년 캐시 → `build-portal.sh`가 빌드마다 `?v=타임스탬프`로 캐시버스트(전 모듈). 재업로드 후 시크릿 새 창(창 전부 닫고)에서 테스트할 것.
...
## ⚠️ 심사 요건 — 첫 Play 광고 (2026-07-07 거절 사유 → 수정됨)
테스터 거절: *"Ads should be shown the first time after the game loads or on the first Play button click."*
→ **GM 빌드에서만** 첫 Play 클릭 시 인터스티셜 1회 노출하도록 구현(`Ads.needsFirstPlayAd`/`showFirstPlayAd`,
main.js onStart 게이팅). 일반 캐던스(3→2→1)는 불변, 2번째 Play부터는 광고 없음. 무필이어도
워치독(15s)이 게임 시작을 보장. CrazyGames·GD·메인 빌드에는 영향 없음(GM.active 게이팅).
